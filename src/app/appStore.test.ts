...
describe('appStore',()=>{
  ...
  it('returns from a result to a clean landing state',()=>{useAppStore.setState({currentStep:'result-report',selectedMissionId:'late-winner',simulationInput:{} as never,simulationResult:{} as never});useAppStore.getState().restart();expect(useAppStore.getState()).toMatchObject({currentStep:'landing',selectedMissionId:null,simulationInput:null,simulationResult:null})})
})
