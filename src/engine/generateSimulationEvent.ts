...
export function generateSimulationEvent(c:GenerateEventContext):SimulationEvent{const control=c.userMetrics.midfieldControl/(c.userMetrics.midfieldControl+c.opponentMetrics.midfieldControl);const userAttacks=c.random.chance(clamp01(control+(c.userMetrics.pressingPower-c.opponentMetrics.pressingPower)/350));const attacking=userAttacks?c.userMetrics:c.opponentMetrics,defending=userAttacks?c.opponentMetrics:c.userMetrics;const attackingTeamId=userAttacks?c.input.teamId:c.input.opponentTeamId
  ...
  const selectedType=c.random.weighted<SimulationEventType>([
    ...
    {value:'box_entry',weight:4+chance*15+finishingEdge*2},{value:'shot',weight:5+chance*15+finishingEdge*2.5},{value:'big_chance',weight:2+chance*9+finishingEdge*3},{value:'turnover',weight:7+turnoverBoost},{value:'counter_attack',weight:4+counterBoost},{value:'foul',weight:5},{value:'card',weight:1.3},{value:'set_piece',weight:3+setPiece*6},{value:'defensive_block',weight:6+defending.defenseStability/16},{value:'save',weight:2+defending.defenseStability/28},{value:'stamina_drop',weight:c.minute>=75?4:1},{value:'goal',weight:1.2+Math.max(0,chance)*5.2+finishingEdge*2.6+(setPiece>.72?1.1:0)}
  ])
  const goalProbability=clamp01(.035+attacking.attackPower/1050+attacking.chanceCreation/1200+attacking.transitionThreat/2200-defending.defenseStability/2400+finishingEdge*.055+lateUrgency+(c.input.instructions.riskLevel==='aggressive'&&userAttacks?0.035:0)+(selectedType==='big_chance'?0.24:selectedType==='shot'?0.105:selectedType==='box_entry'?0.075:selectedType==='set_piece'?0.075:0))
