...
export function generateSimulationEvent(c:GenerateEventContext):SimulationEvent{const control=c.userMetrics.midfieldControl/(c.userMetrics.midfieldControl+c.opponentMetrics.midfieldControl);const userAttacks=c.random.chance(clamp01(control+(c.userMetrics.pressingPower-c.opponentMetrics.pressingPower)/350));const attacking=userAttacks?c.userMetrics:c.opponentMetrics,defending=userAttacks?c.opponentMetrics:c.userMetrics;const attackingTeamId=userAttacks?c.input.teamId:c.input.opponentTeamId
  ...
  const selectedType=c.random.weighted<SimulationEventType>([
    ...
    {value:'box_entry',weight:4+chance*13},{value:'shot',weight:5+chance*13},{value:'big_chance',weight:2+chance*7},{value:'turnover',weight:7+turnoverBoost},{value:'counter_attack',weight:4+counterBoost},{value:'foul',weight:5},{value:'card',weight:1.3},{value:'set_piece',weight:3+setPiece*6},{value:'defensive_block',weight:6+defending.defenseStability/15},{value:'save',weight:2+defending.defenseStability/25},{value:'stamina_drop',weight:c.minute>=75?4:1},{value:'goal',weight:.75+Math.max(0,chance)*3.5+(setPiece>.72?0.8:0)}
  ])
  const goalProbability=clamp01(.025+attacking.attackPower/1400+attacking.chanceCreation/1600-defending.defenseStability/2600+(c.input.instructions.riskLevel==='aggressive'&&userAttacks?0.028:0)+(selectedType==='big_chance'?0.18:selectedType==='shot'?0.07:selectedType==='box_entry'?0.045:selectedType==='set_piece'?0.055:0))
