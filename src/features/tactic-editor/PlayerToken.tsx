...
export function PlayerToken({placement,selected,onSelect}:{placement:PlayerPlacement;selected:boolean;onSelect:()=>void}){
  ...
  return <button ref={node=>{drag.setNodeRef(node);drop.setNodeRef(node)}} {...drag.listeners} {...drag.attributes} onClick={onSelect} style={{left:`${placement.x}%`,top:`${placement.y}%`,transform:`translate(-50%,-50%) ${drag.isDragging?CSS.Translate.toString(drag.transform):''}`,zIndex:drag.isDragging?30:2}} className="absolute touch-none select-none text-center">
