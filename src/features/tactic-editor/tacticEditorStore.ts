...
interface EditorState {
  ...
  selectedPlayerId:string|null;positionOverrides:Record<string,string>
...
export const useTacticEditorStore=create<EditorState>((set,get)=>({
  missionId:null,formationId:'formation-433',players:[],benchPlayerIds:[],substitutions:[],instructions:{...defaultInstructions},selectedPlayerId:null,positionOverrides:{},
  initialize:(missionId)=>{if(get().missionId===missionId)return;const mission=getMission(missionId),team=getTeam(mission.context.userTeamId),lineup=getLineupForMission(missionId);const startingIds=(lineup?.startXI.map(player=>player.id)??team.startingPlayerIds).filter(id=>id!==mission.context.dismissedPlayerId);const benchIds=(lineup?.substitutes.map(player=>player.id)??team.benchPlayerIds).filter(id=>id!==mission.context.dismissedPlayerId&&!startingIds.includes(id));const positionOverrides=[...(lineup?.startXI??[]),...(lineup?.substitutes??[])].reduce<Record<string,string>>((map,player)=>map[player.id]?map:{...map,[player.id]:player.pos??''},{});set({missionId,formationId:mission.recommendedFormationId,players:place(startingIds,mission.recommendedFormationId,mission.context.dismissedPosition,positionOverrides),benchPlayerIds:[...benchIds],substitutions:[],instructions:realInstructions(missionId),selectedPlayerId:startingIds[0]??null,positionOverrides})},
